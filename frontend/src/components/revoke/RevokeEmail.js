    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const response = await fetch(
          "http://127.0.0.1:5000/analyze-email-preview",
          {
            method: "POST",
            body: combinedData,
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const results = await Promise.all([
        emailPreviewAPI(uploadedFile, fileContent),
      ]);
          ...
          <div className="form-container">
            ...
            <div className="form-group">
              ...
              >
                ...
                <option value="NEIGHBOURHOOD_COMPARISON">
            ...
            {message && (
              <div className={`analysis-message ${messageStyle}`}>
                {message}
              </div>
            )}
            {error && (
              <div className={`analysis-message ${messageStyle}`}>{error}</div>
            )}
        ...
        <div className="card">
          ...
          <div className="download-buttons">
            {userEmailDetails.length > 0 && (
              <DownloadCSV
                data={userEmailDetails}
                filename="email_prev_details.csv"
                label="Download Email Preview Details"
              />
            )}
          </div>
              ...
              {showFilter && (
                <div className="filter-popup">
                  <div className="filter-popup-content">
                    <h3>Apply Filter</h3>
                    <p className="filter-column-name">
                      Filtering on: <strong>{filterColumn}</strong>
                    </p>
                    <input
                      type="text"
