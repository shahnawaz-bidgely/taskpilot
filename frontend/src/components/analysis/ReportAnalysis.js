...
function ReportAnalysis() {
  ...
  const callUserDetailsRedshift = async (uploadedFile, fileContent) => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000/analyze-users-redshift",
        {
          method: "POST",
          body: formData,
        }
  ...
  const callClusterDetailsRedshift = async (uploadedFile, fileContent) => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000/analyze-cluster-redshift",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleRedshiftAnalysis = async () => {
    ...
    if (selection.clusterDetails) {
      apiCallsRedshift.push(
        callClusterDetailsRedshift(uploadedFile, fileContent)
      );
    }
    ...
    <div className="report-analysis">
      ...
      <div className="toggle-container">
        ...
        <Switch
          ...
          offColor="#f44336"
          checkedIcon={
            <div style={{ padding: "5px", color: "white" }}>Redshift</div>
          }
          uncheckedIcon={
            <div style={{ padding: "5px", color: "white" }}>API</div>
          }
        />
      ...
      <div className="card">
        ...
        <div className="download-buttons">
          {userDetails.length > 0 && clusterDetails.length > 0 && (
            <button
              onClick={() => downloadAll(userDetails, clusterDetails)}
          ...
          {clusterDetails.length > 0 && (
            <DownloadCSV
              data={clusterDetails}
              filename="cluster_details.csv"
              label="Download Cluster Details"
            />
          )}
        </div>
        ...
        {userDetails.length > 0 && (
          <div className="user-details-section">
            ...
            <table className="user-details-table">
              <thead>
                <tr>
                  {Object.keys(userDetails[0]).map((key, idx) => (
                    <th key={idx}>{key}</th>
                  ))}
                </tr>
              </thead>
        ...
        {clusterDetails.length > 0 && (
          <div className="cluster-details-section">
            ...
            <table className="cluster-details-table">
              <thead>
                <tr>
                  {Object.keys(clusterDetails[0]).map((key, idx) => (
                    <th key={idx}>{key}</th>
                  ))}
                </tr>
              </thead>
